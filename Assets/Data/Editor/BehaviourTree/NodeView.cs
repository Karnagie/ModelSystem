...
using MetaGame.BehaviourTreeNodes;
using UnityEditor;
...
namespace Data.Editor
{
    public class NodeView : UnityEditor.Experimental.GraphView.Node
    {
        ...
        {
            var actionNode = node as CustomActionNode;
            if (actionNode)
            {
                actionNode.name = actionNode.customEventKey;
                title = node.name;
                
                var foldout = new Foldout();
                foldout.text = "On Update";
                foldout.value = false;
                var keys = node.blackboard?.objects.Keys.ToArray();
                if (keys != null)
                {
                    foreach (var key in keys)
                    {
                        ...
                        var place = this.Q<VisualElement>("action-place");
                        place.Add(foldout);
                    }
                }
